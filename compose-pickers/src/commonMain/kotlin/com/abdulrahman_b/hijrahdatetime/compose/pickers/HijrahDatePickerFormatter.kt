...
class HijrahDatePickerFormatter(
     ...
     */
    private val formattersCache = mutableMapOf<String, HijrahDateTimeFormat>()
     ...
     */
    fun formatHeadlineDate(date: HijrahDate?, locale: FormatLocale, decimalStyle: DecimalStyle = DecimalStyle.OfLocale(locale)): String? {
        if (date == null) return null
        return getOrCreateFormatter(selectedDateSkeleton, locale, decimalStyle).let(date::format)
    }
     ...
     */
    internal fun formatDate(date: HijrahDate?, locale: FormatLocale, decimalStyle: DecimalStyle, forContentDescription: Boolean = false): String? {

        if (date == null) return null
        val skeleton = if (forContentDescription) selectedDateDescriptionSkeleton else selectedDateSkeleton

        return getOrCreateFormatter(skeleton, locale, decimalStyle).let(date::format)
    }
     ...
     */
    internal fun formatInputDateWithoutDelimiters(date: HijrahDate?, locale: FormatLocale, decimalStyle: DecimalStyle): String? {
        if (date == null) return null
     ...
     */
    internal fun formatMonthYear(date: HijrahYearMonth?, locale: FormatLocale, decimalStyle: DecimalStyle): String? {
        if (date == null) return null
        return date.format(getOrCreateFormatter(yearMonthSelectionSkeleton, locale, decimalStyle))
    }
     ...
     */
    internal fun parseDateWithoutDelimiters(text: String, locale: FormatLocale, decimalStyle: DecimalStyle): Result<HijrahDate> {
        ...
        return runCatching { HijrahDate.parse(text, formatter) }
    }
