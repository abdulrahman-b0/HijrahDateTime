...
import com.abdulrahman_b.hijrahdatetime.compose.pickers.tokens.MotionTokens
...
internal fun calculateYearMonthFromPage(page: Int, yearsRange: IntRange): HijrahYearMonth {

    val years = yearsRange.first + (page / 12)
    val months = page % 12 + 1

    return HijrahYearMonth(years, months)
}

internal fun calculatePageFromYearMonth(date: HijrahYearMonth, yearsRange: IntRange): Int {
    val years = date.year
    val months = date.month.number
...
internal fun calculateDaysFromStartOfWeekToFirstOfMonth(
    ...
    val firstOfMonth = displayedMonth.onDay(1)
