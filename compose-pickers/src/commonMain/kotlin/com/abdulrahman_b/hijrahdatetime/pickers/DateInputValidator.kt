...
import com.abdulrahman_b.hijrahdatetime.pickers.datepicker.DateInputFormat
import com.abdulrahman_b.hijrahdatetime.pickers.datepicker.InputIdentifier
import com.abdulrahman_b.hijrahdatetime.resources.Res
import com.abdulrahman_b.hijrahdatetime.resources.date_input_invalid_for_pattern
import com.abdulrahman_b.hijrahdatetime.resources.date_input_invalid_not_allowed
import com.abdulrahman_b.hijrahdatetime.resources.date_input_invalid_year_range
...
internal class DateInputValidator(
    ...
    private val errorInvalidRangeInput: StringResource?,
        ...
        return dateToValidate.fold(
            onSuccess = { date ->
                ...
                if (!yearRange.contains(date.year)) {
                    return getString(errorDateOutOfYearRange, yearRange.first, yearRange.last)
                }
                ...
                with(selectableDates) {
                    if (
                        ...
                        return getString(
                            errorInvalidNotAllowed, dateFormatter.formatDate(
                                date = date,
                                locale = locale,
                ...
                if (
                    (inputIdentifier == InputIdentifier.StartDateInput && date >= (currentEndDate ?: HijrahDate.MAX)) ||
                    (inputIdentifier == InputIdentifier.EndDateInput && date < (currentStartDate ?: HijrahDate.MIN))
                ) {
                    // The start date is after the end date, or the end date is before the start date.
                    return@fold errorInvalidRangeInput?.let { getString(it) } ?: ""
                }
            ...
            onFailure = { exception ->
                ...
                return if (exception is IllegalArgumentException && exception.message?.contains("out of range", ignoreCase = true) == true) {
                    ...
                    getString(errorDateOutOfYearRange, firstYear, lastYear)
                } else {
                    getString(errorDatePattern, dateInputFormat.patternWithDelimiters.uppercase())
                }
            }
        )
...
internal fun rememberDateInputValidator(
    ...
    val errorDatePattern = Res.string.date_input_invalid_for_pattern
    val errorDateOutOfYearRange = Res.string.date_input_invalid_year_range
    val errorInvalidNotAllowed = Res.string.date_input_invalid_not_allowed
