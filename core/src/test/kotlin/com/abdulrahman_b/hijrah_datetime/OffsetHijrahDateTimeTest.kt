...
class OffsetHijrahDateTimeTest {
    ...
    @BeforeEach
    fun setUp() {
        offsetHijrahDateTime = OffsetHijrahDateTime.of(1446, 2, 5, 12, 43, 18, offset =  ZoneOffset.of("+03:00"))
    }
    ...
    inner class FactoryTests {
        ...
        fun currentTimeIsObtainedProperly() {
            var actual = OffsetHijrahDateTime.now()
            assertEquals(ZoneOffset.systemDefault().rules.getOffset(actual.toInstant()), actual.offset)

            actual = OffsetHijrahDateTime.now(ZoneOffset.ofHours(3))
        ...
        @Test
        @DisplayName("OffsetHijrahDateTime from TemporalAccessor is obtained properly")
        fun fromTemporalAccessor() {
            var actual = OffsetHijrahDateTime.from(offsetHijrahDateTime)
            assertEquals(offsetHijrahDateTime, actual)

            assertThrows<DateTimeException> {
                OffsetHijrahDateTime.from(HijrahDate.now())
            }
            assertThrows<DateTimeException> {
                OffsetHijrahDateTime.from(Instant.now())
            }

            actual = OffsetHijrahDateTime.from(ZonedDateTime.from(offsetHijrahDateTime))
            assertEquals(offsetHijrahDateTime, actual)
    ...
    inner class FormattingAndParsingTest {
        ...
        fun hijrahDateTimeIsFormattedProperly() {
            var expected = "1446-02-05T12:43:18+03:00"
            var actual = offsetHijrahDateTime.format(HijrahFormatters.HIJRAH_OFFSET_DATE_TIME)
            assertEquals(expected, actual)

            expected =  "1446-02-05+03:00"
            actual = offsetHijrahDateTime.format(HijrahFormatters.HIJRAH_OFFSET_DATE)
            assertEquals(expected, actual)
        }
    ...
    inner class OffsetConversionTest {
        ...
        fun withZoneSameInstant() {
            val newOffset = ZoneOffset.of("+08:00")
            val newOffsetHijrahDateTime = offsetHijrahDateTime.withOffsetSameInstant(newOffset)

            val expected = OffsetHijrahDateTime.of(1446, 2, 5, 17, 43, 18, 0, newOffset)

            assertEquals(expected, newOffsetHijrahDateTime)

            val newOffsetHijrahDateTime2 = offsetHijrahDateTime.withOffsetSameInstant(offsetHijrahDateTime.offset)
            assertEquals(offsetHijrahDateTime, newOffsetHijrahDateTime2)
        }
        ...
        @Test
        @DisplayName("OffsetHijrahDateTime.toHijrahDateTime returns the correct HijrahDateTime instance")
        fun toHijrahDateTime() {
            val hijrahDateTime = offsetHijrahDateTime.toHijrahDateTime()
